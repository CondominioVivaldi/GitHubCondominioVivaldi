...
export default function ReclamosPendientes() {
  ...
  useEffect(() => {
    const cargar = async () => {
      const res = await fetch("/api/reclamos?estado=pendientes", { cache: "no-store" });
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen px-4 sm:px-8 py-10 space-y-8 bg-mi-gradiante-blanco">
      ...
      {/* 📌 Cuadro 1: Tabla */}
      <div className="bg-[var(--Mi-blanco)] w-[950px] max-w-full rounded-2xl shadow-2xl p-6 sm:p-8 animate-fade-in">
          ...
          <div className="bg-[var(--Mi-blanco)] rounded-2xl shadow-2xl p-6 sm:p-8 space-y-6 animate-fade-in">
            ...
            <div>
              ...
              <input
                ...
                readOnly
                className="Mi_texto_20 w-full border border-gray-400 rounded-lg p-2 bg-gray-100"
              />
            </div>
